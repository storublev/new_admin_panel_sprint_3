...
ETL заполняет три индекса, каждый описан пайплайном в `etl/pipelines.py`:

| Индекс | Источник | Документ |
|---|---|---|
| `movies` | `film_work` | фильм с жанрами, актёрами, сценаристами и режиссёрами, дата выхода и метка доступа |
| `genres` | `genre` | жанр (`id`, `name`, `description`) — только жанры, которые есть в фильмах |
| `persons` | `person` | персона (`id`, `full_name`) и её фильмы с ролями — только люди, которые есть в фильмах |

Изменения берутся из `content.audit_log`: по записи любой из таблиц `film_work`, `genre`,
`person`, `genre_film_work`, `person_film_work` определяется, какие документы каких индексов
затронуты (например, при переименовании жанра переиндексируются и все его фильмы).
Затронутые документы перечитываются из PostgreSQL: найденные индексируются, отсутствующие
удаляются из индекса. Начальная загрузка каждого индекса сохраняет прогресс в `content.etl_state`
и после перезапуска продолжается с места остановки.

### Метка доступа фильма

Для сервиса авторизации в документе фильма есть `creation_date` (дата выхода)
и `access_level` — кому доступен фильм:

* `subscription` — фильм вышел менее `SUBSCRIPTION_PERIOD_YEARS` лет назад
  (по умолчанию трёх) или ещё не вышел: смотреть его могут только пользователи
  с правом `films.subscription` (роль `subscribers`);
* `public` — остальные фильмы и фильмы без даты выхода: доступны всем.

Правило — в `etl/access.py`. Метка вычисляется при индексации и сама не
меняется, а фильм выходит из новинок с течением времени, поэтому раз в сутки
ETL переиндексирует фильмы, у которых срок новинки истёк с прошлой проверки
(дата проверки — `access_levels_date` в `content.etl_state`).

Если в схеме индекса появились новые поля, ETL добавляет их в маппинг
существующего индекса и загружает его документы заново — пересоздавать
индекс вручную не нужно.

Тесты метки (из каталога `etl`): `pip install pytest && python -m pytest tests`.
